...
cmdlnPrintHandler.prototype = {
    ...
    handle: function(cmdLine) {
        const handler = new cmdlPrintRealHandler();
        try {
            const options = handler.retrieveOptionsFromCmdLine(cmdLine);
...
cmdlPrintRealHandler.prototype = {
    ...
    argumentBoolean: function(value, defaultValue) {
        ...
        if ('yes' === value || 'y' === value || '1' === value || 'on' === value) {
            return 'yes';
        }
        if ('no' === value || 'n' === value || '0' === value || 'off' === value) {
            return 'no';
        }
        return defaultValue;
    },
    ...
    argumentMargins: function (definition, defaultValue) {
        ...
        const top = this.argumentInteger(parts[0], defaultValue);
        const right = (parts.length > 1) ? this.argumentInteger(parts[1], top) : top;
        const bottom = (parts.length > 2) ? this.argumentInteger(parts[2], top) : top;
        const left = (parts.length > 3) ? this.argumentInteger(parts[3], right) : right;
        return {
            top: top,
            right: right,
            bottom: bottom,
    ...
    retrieveOptionsFromCmdLine: function (cmdline) {
        const printParameter = this.argumentString(cmdline.handleFlagWithParam('print', false), '');
        if ('' === printParameter) {
            return null;
        }
        ...
        const aMargins = this.argumentString(cmdline.handleFlagWithParam('print-margins', false), '');
        if ('' !== aMargins) {
            const borderMargins = this.argumentMargins(aMargins, 10);
        ...
        const aEdges = this.argumentString(cmdline.handleFlagWithParam('print-edges', false), '');
        if ('' !== aEdges) {
            const borderEdges = this.argumentMargins(aEdges, 10);
        ...
        const uri = cmdline.resolveURI(options.print);
        ...
        if ('printer' !== options.mode) {
            ...
            const aFileURI = cmdline.resolveURI(options.file);
    ...
    optionsToWindowArguments: function (options) {
        const windowArguments = Cc['@mozilla.org/supports-array;1'].createInstance(Ci.nsICollection);
        for (let key in options) {
